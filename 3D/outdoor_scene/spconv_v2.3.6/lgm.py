...
for scene_iter, (coords_pcl, labels, sig) in enumerate(val_pt_dataset):
    ...
    for iter in range(attack_configs.iter_num):
        ...
        with torch.no_grad():
            model.eval()
            coords_pcl_orig, coords_vox_orig, feats_orig = \
            ...
            if iter == 0:
                mIoU0 = iou_orig
                coords_pcl_best = coords_pcl_orig.cpu().detach().numpy()
        ...
        outputs = model([feats], [coords_vox], val_batch_size, occupy=occupy)
