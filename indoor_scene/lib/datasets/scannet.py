...
class ScannetVoxelizationDataset(VoxelizationDataset):
  ...
  def __init__(self,
    ...
    data_paths = data_paths = read_txt(os.path.join('/data3/antao/Documents/Datasets/ScanNet_processed/', self.DATA_PATH_FILE[phase]))
    logging.info('Loading {}: {}'.format(self.__class__.__name__, self.DATA_PATH_FILE[phase]))
    if phase == DatasetPhase.Val:
      data_root = os.path.join('/data3/antao/Documents/Datasets/ScanNet_processed/', 'train')
    elif phase == DatasetPhase.Test:
      data_root = os.path.join('/data3/antao/Documents/Datasets/ScanNet_processed/', 'test')
    super().__init__(
        data_paths,
        data_root=data_root,
