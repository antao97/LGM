...
class RNNGate(nn.Module):
    ...
    def init_hidden(self, batch_size):
        ...
        #                                       self.hidden_dim).cuda()))
        return (torch.zeros(1, batch_size, self.hidden_dim, requires_grad=False).cuda(),
                torch.zeros(1, batch_size, self.hidden_dim, requires_grad=False).cuda())
    ...
    def forward(self, x):
        ...
        prob = prob.view(batch_size, 1, 1, 1)
    ...
    def forward(self, x):
        ...
        for g in range(3):
            for i in range(0 + int(g == 0), self.num_layers[g]):
                ...
                prev = x = gprob.expand_as(x) * x \
                           + (1 - gprob).expand_as(prev) * prev
...
class RNNGatePolicy(nn.Module):
    def __init__(self, input_dim, hidden_dim, rnn_type='lstm', lamb=10, diff_type='gg'):
    ...
    def forward(self, x):
        ...
        proj = self.proj(out.squeeze())
        prob = self.prob(self.lamb*proj)
        bi_prob = torch.cat([1 - prob, prob], dim=-1)
        if self.diff_type == 'fg':
            action = (prob > 0.5).float().detach() - \
                    prob.detach() + prob
        elif self.diff_type == 'gg':
            action = prob
        self.saved_actions.append(action)
...
class ResNetRecurrentGateRL(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10,
                 embed_dim=64, hidden_dim=64, lamb=10, diff_type='gg'):
        self.inplanes = 16
        ...
        self.control = RNNGatePolicy(embed_dim, hidden_dim, lamb=self.lamb, diff_type=self.diff_type)
...
# for CIFAR-10
def cifar10_rnn_gate_rl_38(pretrained=False, lamb=10, diff_type='gg', **kwargs):
    """SkipNet-38 + RL with Recurrent Gate"""
    model = ResNetRecurrentGateRL(BasicBlock, [6, 6, 6], num_classes=10,
                                  embed_dim=10, hidden_dim=10, 
                                  lamb=lamb, diff_type=diff_type)
    return model


def cifar10_rnn_gate_rl_74(pretrained=False, lamb=10, diff_type='gg', **kwargs):
...
def cifar10_rnn_gate_rl_110(pretrained=False, lamb=10, diff_type='gg', **kwargs):
...
# for CIFAR-100
def cifar100_rnn_gate_rl_38(pretrained=False, lamb=10, diff_type='gg', **kwargs):
    """SkipNet-38 + RL with Recurrent Gate"""
...
def cifar100_rnn_gate_rl_74(pretrained=False, lamb=10, diff_type='gg', **kwargs):
    """SkipNet-74 + RL with Recurrent Gate"""
...
def cifar100_rnn_gate_rl_110(pretrained=False, lamb=10, diff_type='gg', **kwargs):
    """SkipNet-110 + RL with Recurrent Gate"""
