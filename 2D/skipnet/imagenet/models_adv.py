...
class RNNGate(nn.Module):
    ...
    def forward(self, x):
        ...
        prob = self.prob(proj)
        prob = (prob > 0.5).float().detach() - prob.detach() + prob
    ...
    def forward(self, x):
        ...
        for g in range(4):
            for i in range(0 + int(g == 0), self.num_layers[g]):
                ...
                prev = x = gprob.expand_as(x)*x + (1-gprob).expand_as(prev)*prev
...
class RNNGatePolicy(nn.Module):
    def __init__(self, input_dim, hidden_dim, rnn_type='lstm', lamda=10, diff_type='gg'):
    ...
    def forward(self, x):
        ...
        proj = self.proj(out).view(out.size(0))
        prob = self.prob(self.lamda*proj)
        ...
        if self.diff_type == 'fg':
            action = (prob > 0.5).float().detach() - prob.detach() + prob
...
class RecurrentGatedRLResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, embed_dim=10,
                 hidden_dim=10, lamda=10, diff_type='gg', **kwargs):
        self.inplanes = 64
        ...
        self.control = RNNGatePolicy(embed_dim, hidden_dim, rnn_type='lstm', lamda=self.lamda, diff_type=self.diff_type)
...
def imagenet_rnn_gate_rl_18(pretrained=False, lamda=10, diff_type='gg', **kwargs):
    ...
    model = RecurrentGatedRLResNet(BasicBlock, [2, 2, 2, 2], embed_dim=10,
                                   ...
                                   lamda=lamda, diff_type=diff_type)
    return model


def imagenet_rnn_gate_rl_34(pretrained=False, lamda=10, diff_type='gg', **kwargs):
...
def imagenet_rnn_gate_rl_50(pretrained=False, lamda=10, diff_type='gg', **kwargs):
...
def imagenet_rnn_gate_rl_101(pretrained=False, lamda=10, diff_type='gg', **kwargs):
...
def imagenet_rnn_gate_rl_152(pretrained=False, lamda=10, diff_type='gg', **kwargs):
